...
class Slider():
    ...
    def __init__(self, game, name, alias, val, val_min, val_max, x_pos, y_pos):
        ...
        self.txt_surface = self.font.render(alias, 1, (255, 255, 255))
    ...
    def move(self):
        ...
        if self.name in overwrite_settings:
            if self.name[3:] != 'max' and int(self.val) > self.game.board.options[f'{self.name[:1]}_max']['val']:
                self.game.board.options[f'{self.name[:1]}_max']['val'] = int(self.val)
            elif int(self.val) < self.game.board.options[f'{self.name[:1]}_min']['val']:
                self.game.board.options[f'{self.name[:1]}_min']['val'] = int(self.val)

        self.game.board.options[self.name]['val'] = int(self.val)

        if self.name == 'cell_minimum':
